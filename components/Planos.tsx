...
import { Telephone, Send, Reception4, InfoCircle, ChatLeftTextFill, Cast } from 'react-bootstrap-icons';
...
const Planos: React.FC = () => {
  ...
  return (
    <Row className={`justify-content-center ${stylesTariffs.main}`}>
      <Typography variant="h4" className={` text-center m-5 ${stylesTariffs.titleTariffs} `}>Planos para quem quer mais internet</Typography>
      {tariffs.map((plan, index) => {
        // Verifica se o nome do plano não contém a palavra "Basic"
        if (!plan.nome.toLowerCase().includes('basic')) {
          return (
            <Col sm={12} md={6} lg={3} className="col-11 mx-0 px-0 position-relative mt-5" key={index}>
              <div className={`${stylesTariffs.badgeContainer}`}>
                {plan.gbFixo === 20 &&
                  <Badge className={stylesTariffs.maisVendido}>Mais Vendido</Badge>
                }
              </div>
              <Card className={stylesTariffs.cardPlanos}>
                <Card.Header className={stylesTariffs.cardPlanosHeader}>
                  <Card.Title as="h5" className={stylesTariffs.cardPlanosNome}>{plan.nome}</Card.Title>
          ...
          return null; // Não renderiza o plano se contém "Basic" no nome
        }
      })}
    </Row>
  );
